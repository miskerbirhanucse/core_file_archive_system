        ...
        <div class="card shadow p-3 mb-5 bg-white rounded">
            <div class="card-body ">
                ...
                <form id="fileUploadForm" class="forms-sample" enctype="multipart/form-data" method="POST"
                      action="{{ route('letter.update.secretary',['id'=>$letter->id])}}">
                    @method('put')
                    @csrf
                    <div class="form-group">
                        ...
                        <select name="department_id" class="form-control bg-white text-dark" required>
                            ...
                            @foreach($departments as $department)
                                <option value="{{$department->id}}" {{$letter->department_id==$department->id?'selected':''}}>{{$department->name}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <select name="project_id" class="form-control bg-white text-dark" required>
                            ...
                            @foreach($projects as $project)
                                <option
                                    value="{{$project->id}}"{{$letter->project_id==$letter->id?'selected':''}}>{{$project->name}}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control bg-white text-dark" name="ref" id="exampleInputName1"
                               value="{{$letter->ref_no}}" placeholder="" required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea type="text" class="form-control bg-white text-dark" name="remark"
                                  id="exampleInputName1" placeholder="">{{$letter->remark}}</textarea>
                    </div>
                    <div class="form-group">
                        @error('letter_path')
