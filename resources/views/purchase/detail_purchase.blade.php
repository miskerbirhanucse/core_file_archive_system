        ...
        <div class="card shadow p-3 mb-5 bg-white rounded">
            <div class="card-body ">
                ...
                <div class="form-group">
                    <p for="exampleInputName1" class="text-dark display-5 ">Requested By:
                        <b>{{$purchase->user->name}} </b></p>
                </div>
                <div class="form-group">
                    <p for="exampleInputEmail3" class="text-dark display-5 ">From
                        Department:<b> {{$purchase->department->name}}</b></p>

                </div>

                <div class="form-group">
                    <p for="exampleInputEmail3" class="text-dark display-5">Description:
                        <b>{{$purchase->description}}</b></p>
                </div>
                <div class="form-group">
                    <p for="exampleInputEmail3" class="text-dark display-5">Specification:
                        <b>{{$purchase->specification }}</b></p>
                </div>
                ...
                <div class="form-group">
                    <p for="exampleInputEmail3" class="text-dark display-5">Estimated Cost:
                        <b>{{number_format($purchase->estimated_cost)}}</b></p>
                </div>

                <div class="form-group">
                    <p for="exampleInputEmail3" class="text-dark display-5">Project Name:
                        <b>{{$purchase->project_name}}</b></p>
                </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-8">
                        <div class="form-group">
                            <p for="exampleInputEmail3" class="text-dark display-5">Approved By Property Administration
                                Department:
                ...
                <form action="{{ route('approve.purchase',['id'=>$purchase->id ])}}" method="POST" id="approve">
                            ...
                            <div class="form-group">
                                <div class="form-check form-check-success">
                                    <label class="form-check-label">
                                        <input type="checkbox" name="approve[]" class="form-check-input" value="1">
                                        Approve
                                    </label>
                                </div>
                                <div class="form-check form-check-danger">
                                    <label class="form-check-label">
                                        <input type="checkbox" name="approve[]" class="form-check-input" value="2">
                                        Reject
                                    </label>
                                </div>
                            </div>
                    ...
                    @if($purchase->user_id == auth()->user()->id)
                        <a type="submit" class="btn btn-primary mr-2 mb-5 "
                           href="{{ route('edit.purchase',['id'=>$purchase->id ])}}">Edit</a>
                    @endif
                    @can('approve request')
                    ...
                    @can('authorize request')
                        <div class="col-md-6">
                            ...
                            <div class="form-group">
                                <div class="form-check form-check-success">
                                    <label class="form-check-label">
                                        <input type="checkbox" name="authorize[]" class="form-check-input" value="1">
                                        Authorize
                                    </label>
                                </div>
                                <div class="form-check form-check-danger">
                                    <label class="form-check-label">
                                        <input type="checkbox" name="authorize[]" class="form-check-input" value="2">
                                        Reject
                                    </label>
                                </div>
                            </div>
                ...
                @can('authorize request')
                    <button type="submit" form="authorize" class="btn btn-primary mr-2" onsubmit="authorize">Authorize
                    </button>
                @endcan
                @if($purchase->approved_by_store_id == null)
                    <form action="{{ route('store.approve.purchase',['id'=>$purchase->id ])}}" method="post"
                          id="storeApprove">
                        @csrf
                        {{method_field('PUT')}}
                        @can('export request')
                            <div class="col-md-6">
                                ...
                                <div class="form-group">
                                    <div class="form-check form-check-success">
                                        <label class="form-check-label">
                                            <input type="checkbox" name="storeApprove[]" class="form-check-input"
                                                   value="1"> Yes
                                        </label>
                                    </div>
                                    <div class="form-check form-check-danger">
                    ...
                    <button type="submit" form="storeApprove" class="btn btn-primary mr-2" onsubmit="authorize">
                        Approve
                    </button>
                @endcan
                @endif
                <br>
                <br>
                @if($purchase->approved_by_store === 0 && $purchase->authorize===1 )
                    <form action="{{ route('store.isPurchased',['id'=>$purchase->id ])}}" method="post"
                          id="isPurchased">
                        @csrf
                        {{method_field('PUT')}}
                        @can('export request')
